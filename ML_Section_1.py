...
def visualization_page():
    ...
    # ── Dataset Quick Stats Bar ───────────────────────────────────────────────
    st.markdown("""
    <div class="content-frame" style="background:linear-gradient(135deg,#f0f4f8,#ffffff);
         border-left:5px solid #3498db; padding:14px 20px; margin-bottom:16px;">
        <h5 style="margin:0; font-family:Sora,sans-serif; color:#1a2535;">
            🗂️ Active Dataset Overview
        </h5>
    </div>
    """, unsafe_allow_html=True)
        ...
        if not x_opts_ts or not numeric_cols:
            ...
            with settings_col:
                ...
                if chart_type_ts == "Line Chart with Range Selector":
                    ...
                    h_c   = st.selectbox("High",  numeric_cols, key="ts_can_h",
                                         index=min(1, len(numeric_cols)-1))
                    l_c   = st.selectbox("Low",   numeric_cols, key="ts_can_l",
                                         index=min(2, len(numeric_cols)-1))
                    cl_c  = st.selectbox("Close", numeric_cols, key="ts_can_c",
                    ...
                    if x_ts and y_ts:
                        try:
                            ...
                            if n < period * 2:
                                ...
                                fig.update_layout(title="Seasonal Decomposition (Manual)",
            ...
            with settings_col:
                ...
                if chart_type_stat == "Q-Q Plot":
                    ...
                    if col_qq:
                        ...
                        fig.update_layout(
                            title=f"Q-Q Plot: {col_qq} vs {dist_qq}",
                    ...
                    sel_dist = st.multiselect("Numeric Columns to Compare", numeric_cols,
                                              default=numeric_cols[:min(3, len(numeric_cols))],
                                              key="stat_dist_cols")
                    norm_each = st.checkbox("Normalize each column", value=True, key="stat_dist_norm")
                    if sel_dist:
                        fig = go.Figure()
                    ...
                    if col_h:
                        fig = px.histogram(df, x=col_h, color=color, nbins=bins,
                                           ...
                                           color_discrete_sequence=px.colors.qualitative.Set2,
                                           opacity=0.75)
                        st.caption("📌 **When to use:** Full picture of distribution shape.")
                    ...
                    if len(numeric_cols) < 2:
                        ...
                        if target_fi and feature_cols_fi:
                            corrs = df[feature_cols_fi].corrwith(df[target_fi]).abs().sort_values(ascending=False)
                            fig   = px.bar(x=corrs.index, y=corrs.values,
                                           labels={'x':'Feature','y':'|Correlation|'},
                                           title=f"Feature Correlation with '{target_fi}'",
                                           template="plotly_white",
                                           color=corrs.values, color_continuous_scale='Blues')
                            fig.update_layout(height=420)
                            st.caption("📌 **When to use:** Quick feature relevance screening.")
        ...
        if not cat_cols and not numeric_cols:
            ...
            with settings_col:
                ...
                if chart_type_geo == "Choropleth Map (Country)":
                    ...
                    cscale = st.selectbox("Color Scale",
                                          ["Blues","Reds","Greens","Viridis","Plasma","YlOrRd"],
                                          key="geo_choro_cs")
                    if loc and val:
                        try:
                    ...
                    cscale = st.selectbox("Color Scale",
                                          ["Blues","Reds","Greens","Viridis","YlOrRd"],
                                          key="geo_us_cs")
                    if loc and val:
                        try:
                            fig = px.choropleth(df, locations=loc, color=val,
                    ...
                    lon   = st.selectbox("Longitude Column", numeric_cols, key="geo_bub_lon",
                                         index=min(1, len(numeric_cols)-1))
                    size  = st.selectbox("Size (Numeric)", numeric_cols, key="geo_bub_sz",
                                         index=min(2, len(numeric_cols)-1))
                    ...
                    lon   = st.selectbox("Longitude Column", numeric_cols, key="geo_sg_lon",
                                         index=min(1, len(numeric_cols)-1))
        ...
        with settings_col:
            ...
            if chart_type_adv == "Waterfall Chart":
                ...
                if not cat_cols or not date_cols:
                    ...
                    end_col   = st.selectbox("End Date",   date_cols, key="adv_gantt_end",
                                             index=min(1, len(date_cols)-1))
                ...
                if len(numeric_cols) < 2 or not cat_cols:
                    ...
                    col_c  = st.selectbox("Column (X-Axis Category)", cat_cols, key="adv_hm_col",
                                          index=min(1, len(cat_cols)-1))
                    val_c  = st.selectbox("Value (Numeric)", numeric_cols, key="adv_hm_val")
                    agg_hm = st.selectbox("Aggregation", ["mean","sum","count"], key="adv_hm_agg")
                    cscale = st.selectbox("Color Scale", ["Blues","Reds","Greens","Viridis","YlOrRd"],
                                          key="adv_hm_cs")
                ...
                if len(numeric_cols) < 2:
                    ...
                    y_dc   = st.selectbox("Y-Axis", numeric_cols, key="adv_dc_y",
                                          index=min(1, len(numeric_cols)-1))
                ...
                if not cat_cols or not numeric_cols:
                    ...
                    tgt_col = st.selectbox("Target (Category)", cat_cols, key="adv_sk_tgt",
                                           index=min(1, len(cat_cols)-1))
                    val_col = st.selectbox("Flow Value (Numeric)", numeric_cols, key="adv_sk_val")
                    max_n   = st.slider("Max source categories", 3, 15, 8, key="adv_sk_maxn")
    ...
    with tab_group:
            ...
            with col1:
                if not cat_cols or not numeric_cols:
                    ...
                    chart_for_grp = st.selectbox("Visualize As",
                                                  ["Bar Chart","Horizontal Bar","Pie Chart","Treemap"],
                                                  key="grp_chart")
            ...
            with col2:
                if 'grp_df' in dir() and group_cols and value_col and len(grp_df) > 0:
                    ...
                    if chart_for_grp == "Bar Chart":
                        ...
                        fig_g.update_traces(texttemplate='%{text:.3s}', textposition='outside')
        ...
        # ── Pivot Table ──────────────────────────────────────────────────────
        # Initialize pivot variables to None to prevent UnboundLocalError
        ...
        with sub_pivot:
            ...
            with col1:
                if not cat_cols or not numeric_cols:
                    ...
                    piv_cols   = st.selectbox("Columns", cat_cols, key="piv_cols",
                                              index=min(1, len(cat_cols)-1))
                    ...
                    colorscale = st.selectbox("Heatmap Color",
                                              ["Blues","Reds","Greens","YlOrRd","Viridis"],
                                              key="piv_cs")
                ...
                if piv_rows and piv_cols and piv_val and 'piv_table' in dir():
                    try:
                        ...
                        fig_piv = px.imshow(display_piv,
                                            ...
                                            template="plotly_white")
                        fig_piv.update_layout(height=480)
...
def ml_algorithm_page():
    """Advanced Machine Learning Expert System — DataMate AI"""
    import pandas as pd
    import numpy as np
    import time
    import joblib
...
def export_page():
    ...
    class PDFReport(FPDF):
        ...
        def kv_row(self, key, value, shade=False):
            ...
            self.cell(0, 7, str(self.safe(value)), 0, 1)
    ...
    kpi_data = [
        ...
        ("Columns",       f"{len(df.columns)}",              "#2ecc71"),
        ("Missing Cells", f"{df.isnull().sum().sum():,}",    "#e74c3c" if df.isnull().sum().sum() > 0 else "#2ecc71"),
        ("Saved Charts",  f"{len(st.session_state.get('saved_charts', []))}",  "#9b59b6"),
        ("Dataset Name",  str(st.session_state.get('selected_file_name', 'N/A'))[:18], "#e67e22"),
    ]
    ...
    # ══════════════════════════════════════════════════════════════════════════
    with tab_pdf:
        st.markdown('<div class="section-header">📄 Multi-Page PDF Report</div>',
                    unsafe_allow_html=True)
        ...
        with col1:
            ...
            include_profile   = st.checkbox("Column Profile",      value=True, key="pdf_profile")
        ...
        with col2:
            ...
            <div class="content-frame" style="background:linear-gradient(135deg,#f8f9fa,#fff);
                ...
                <ul style="color:#555; font-size:13px; line-height:2;">
                    ...
                    <li>Auto page breaks &amp; headers/footers</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
            ...
            with st.spinner("🔄 Building PDF — this may take a moment for large datasets..."):
                try:
                    ...
                    if include_stats:
                        ...
                        stats_df = df.describe(include='all').round(3)
                        pdf.set_font('Courier', '', 7)
                        pdf.multi_cell(0, 4.5, pdf.safe(stats_df.to_string()))

                    # ── SECTION: COLUMN PROFILE ───────────────────────────────
                    if include_profile:
                        pdf.add_page()
                        pdf.section_title("Column Profile")
                        for col in df.columns[:30]:  # cap at 30
                    ...
                    if include_charts and saved_charts:
                        ...
                        for i, chart in enumerate(saved_charts):
                            ...
                            try:
                                ...
                                pdf.body('[Chart skipped: install kaleido — pip install kaleido]')
                            except Exception as e:
                    ...
                    st.success(f"✅ PDF ready — {len(pdf_bytes)/1024:.1f} KB across "
                               f"{pdf.page} pages.")
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # TAB 2 – HTML DASHBOARD
    # ══════════════════════════════════════════════════════════════════════════
    with tab_html:
        st.markdown('<div class="section-header">🌐 Interactive HTML Dashboard</div>',
                    unsafe_allow_html=True)
        ...
        with h_col1:
            ...
            report_author = st.text_input("Author / Team",  value="DataMate AI",             key="html_author")
        with h_col2:
        ...
        if st.button("⚙️ Generate HTML Dashboard", key="btn_gen_html",
                     use_container_width=True, type="primary"):
            with st.spinner("Building interactive HTML..."):
                html_content = generate_html_report(df, st.session_state.get('saved_charts', []))
                # Inject custom title / author into the header
    ...
    # ══════════════════════════════════════════════════════════════════════════
    with tab_data:
        st.markdown('<div class="section-header">📊 Export Cleaned Dataset</div>',
                    unsafe_allow_html=True)
        ...
        with d_col1:
            ...
            fmt = st.radio("Choose format", ["CSV", "Excel (.xlsx)", "JSON", "Parquet"],
                           key="data_export_fmt")
            max_rows = st.slider("Max rows to export (0 = all)",
                                 0, min(100_000, len(df)), 0, key="data_export_rows")
            export_df = df if max_rows == 0 else df.head(max_rows)

            st.info(f"Exporting **{len(export_df):,}** rows × "
                    f"**{len(export_df.columns)}** columns")

            fname_base = f"cleaned_{st.session_state.get('selected_file_name', 'data')}"
    ...
    # ══════════════════════════════════════════════════════════════════════════
    with tab_txt:
        st.markdown('<div class="section-header">📝 Plain-Text Summary Report</div>',
                    unsafe_allow_html=True)
        ...
        with t_col1:
            ...
            include_desc   = st.checkbox("Descriptive Statistics",   value=True, key="txt_desc")
            ...
            if st.button("⚙️ Generate TXT Summary", key="btn_gen_txt",
                         use_container_width=True, type="primary"):
                sep = "=" * 60
                lines = [
...
def history_page():
    ...
    for col_ui, (label, val, color) in zip(
        ...
        [
            ...
            ("Pinned Charts",    str(total_charts),  "#9b59b6"),
            ("Models Trained",   str(ml_trained),    "#2ecc71"),
            ("Active Dataset",   active_file[:16],   "#e67e22"),
        ]
    ):
        with col_ui:
            st.markdown(f"""
    ...
    with tab_clear:
        ...
        with clear3:
            ...
            """, unsafe_allow_html=True)
            confirm_reset = st.checkbox("I understand this is irreversible", key="confirm_full_reset")
            if st.button("🔴 Full Reset", key="full_reset_btn",
                         use_container_width=True, disabled=not confirm_reset):
                ...
                st.session_state.current_page = 'Welcome'
                st.success("✅ Session fully reset. Redirecting to Welcome...")
                st.rerun()
...
def about_page():
    ...
    with hero_col2:
        st.markdown("""
        <div class="content-frame" style="text-align:center; padding:28px;">
            <div style="font-size:70px; line-height:1;">🤖</div>
            <p style="font-family:Sora,sans-serif; font-weight:800; font-size:18px;
               color:#1a2535; margin:10px 0 4px 0;">DataMate AI</p>
            <p style="color:#7f8c8d; font-size:13px; margin:0;">
                Your smart data partner
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    for row in rows:
        ...
        for col_ui, (icon, title, desc) in zip(cols, row):
            with col_ui:
                st.markdown(f"""
                <div class="feature-card" style="min-height:170px;">
                    <span class="feature-icon">{icon}</span>
                    <div class="feature-title">{title}</div>
                    <div class="feature-desc">{desc}</div>
                </div>
    ...
    for col_ui, (name, email, avatar, role) in zip([f_col1, f_col2], founder_data):
        with col_ui:
            st.markdown(f"""
            <div class="founder-card" style="text-align:center; padding:28px 24px;
                 border-radius:16px; background:linear-gradient(135deg,#f8f9fa,#ffffff);
                 box-shadow:0 6px 24px rgba(0,0,0,0.09); border-top:4px solid #3498db;">
                <div style="font-size:52px; margin-bottom:10px;">{avatar}</div>
                <h3 style="font-family:Sora,sans-serif; margin:0 0 4px 0;
                    color:#1a2535; font-size:18px;">{name}</h3>
                <p style="color:#7f8c8d; font-size:13px; margin:0 0 12px 0;">{role}</p>
                <div style="background:#eaf4fb; padding:8px 16px; border-radius:20px;
                     display:inline-block;">
                    <span style="color:#2980b9; font-weight:600; font-size:13px;">
                        📧 {email}
                    </span>
                </div>
            </div>
    ...
    with v_col1:
        st.markdown("""
        <div class="metric-card" style="border-left:4px solid #2ecc71; text-align:center;">
            <p style="font-size:11px;color:#7f8c8d;text-transform:uppercase;
                      letter-spacing:1px;margin:0;">Version</p>
            <p style="font-size:24px;font-weight:800;color:#2ecc71;
                      margin:6px 0 0 0;font-family:Sora,sans-serif;">v1.0</p>
            <p style="font-size:11px;color:#7f8c8d;margin:4px 0 0 0;">
                Production Release
            </p>
        </div>
        """, unsafe_allow_html=True)
    with v_col2:
        st.markdown("""
        <div class="content-frame" style="border-left:4px solid #3498db;">
            <h4 style="font-family:Sora,sans-serif; margin-top:0; color:#1a2535;">
                💬 Support & Feedback
            </h4>
            <p style="color:#555; font-size:14px; margin:0; line-height:1.7;">
                For bugs, feature requests, or general feedback, reach out to either
                co-founder directly via email. We typically respond within 24–48 hours.
                Your feedback helps us improve DataMate AI for everyone.
            </p>
        </div>
        """, unsafe_allow_html=True)
